...
public class Noise
{
    ...
    public Noise(int seed)
    {
        ...
        for (int i = 0; i < 512; i++)
        {
            if(i < 256)
                ...
                hash[i] = newHash[i - 256];
        }
    }

    public byte Noise1D(int x)
    {
        return hash[x & hashMask];
    }

    public byte Noise2D(int x,int y)
    {
        return Noise1D(Noise1D(x) + y);
    }

    public byte Noise3D(int x, int y, int z)
    {
        return Noise1D(Noise2D(x, y) + z);
    }

    public float Value1D(Vector3 point)
    {
    ...
    public float Value2D(Vector3 point)
    {
    ...
    public float Value3D(Vector3 point)
    {
        int ix0 = Mathf.FloorToInt(point.x);
        int iy0 = Mathf.FloorToInt(point.y);
    ...
    private const float sqr2 = 1.41421356f;
    ...
    public float Sum(NoiseMethod method, Vector3 point, float frequency, int octaves, float freqChange, float ampChange)
    {
